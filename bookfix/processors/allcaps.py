...
class AllCapsProcessor:
    ...
    def process_all_caps_sequences(self, ctx: 'BookfixContext') -> 'BookfixContext':
        ...
        # Ensure ignore_set exists
        if not hasattr(ctx, 'ignore_set'):
            ctx.ignore_set = set(ctx.cap_ignore) if hasattr(ctx, 'cap_ignore') else set()
        
        # Ensure lowercase_set exists
        if not hasattr(ctx, 'lowercase_set'):
            ctx.lowercase_set = set(ctx.upper_to_lower) if hasattr(ctx, 'upper_to_lower') else set()
